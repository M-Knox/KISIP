...
st.markdown('<div class="pg-sub">Observed SCMI (Change Vector Analysis) per 50 m grid zone. SHAP values show which pre-intervention features drove the magnitude of surface change detected.</div>', unsafe_allow_html=True)
...
n_zones   = len(s_gdf)
dom_feat  = s_gdf["dominant_feature"].value_counts().idxmax() if "dominant_feature" in s_gdf.columns else "—"
...
<div class="stat-row">
  ...
  <div class="stat-cell">
    <div class="sc-val" style="font-size:1.1rem;padding-top:0.25rem;">{dom_feat.replace('shap_','')}</div>
    ...
    for _, row in s_gdf.iterrows():
        ...
        folium.GeoJson(
            ...
            tooltip=folium.Tooltip(
                ...
                f"Dominant feature: {dom.replace('shap_','')}",
                sticky=False,
            ),
        ).add_to(m)
...
with col_right:
    ...
    st.markdown('<div style="font-size:0.78rem;color:#8B95A8;margin-bottom:0.6rem;">Mean absolute SHAP value across all zones in this settlement — shows which pre-intervention features most drove change magnitude.</div>', unsafe_allow_html=True)
    ...
    bar_colors = ["#4FC3A1" if i >= len(shap_vals) - 3 else "#2A5A4A"
...
# ── Expandable details ────────────────────────────────────────────────────────
FEAT_COLS = ["NDVI","NDBI","MNDWI","Contrast","Entropy","Homogeneity","Correlation","road_density","paved_proportion"]

with st.expander("Zone feature matrix", expanded=False):
    display_cols = ["zone_id"] + [c for c in FEAT_COLS if c in s_feats.columns]
    st.dataframe(s_feats[display_cols].set_index("zone_id").round(5), use_container_width=True, height=260)
