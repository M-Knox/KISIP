...
MODEL_COLORS = {"Ridge Regression": "#4FC3A1", "Random Forest": "#F5A623", "XGBoost": "#9B7FD4", "Ridge": "#4FC3A1"}
...
st.markdown('<div class="pg-sub">Leave-One-Settlement-Out cross-validation metrics for Ridge Regression, Random Forest, and XGBoost. CVA and PCA compared as SCMI computation methods.</div>', unsafe_allow_html=True)
...
with col_right:
    ...
    st.markdown('<div style="font-size:0.78rem;color:#8B95A8;margin-bottom:0.6rem;">CVA (Change Vector Analysis) vs PCA as the SCMI computation method. Lower RMSE and comparable R² support CVA as the preferred method.</div>', unsafe_allow_html=True)

    fig_cp = go.Figure()
    for method, color in [("CVA","#4FC3A1"), ("PCA","#F5A623")]:
        sub = cva_pca[cva_pca["SCMI_Method"] == method]
        fig_cp.add_trace(go.Bar(
            name=method, x=sub["Model"], y=sub["R2"],
            marker_color=color,
            text=sub["R2"].round(4), textposition="outside", textfont_size=10,
    ...
    if not shap_imp.empty and "mean_abs_shap" in shap_imp.columns:
        ...
        imp_colors = ["#4FC3A1" if i >= len(fi) - 3 else "#2A5A4A" for i in range(len(fi))]
...
with fc1:
    st.markdown(f"""
    <div class="finding">
        <b>CVA outperforms PCA as the SCMI method.</b> CVA-based SCMI keeps RMSE below 0.04 across
        all three models; PCA inflates RMSE above 0.48, indicating sensitivity to the rotation choice.
    </div>
    <div class="finding">
...
with fc2:
    ...
    <div class="finding">
        ...
        values in many pre-intervention zones, suppressing their SHAP contribution — a known gap
        for the 2009–2011 period.
    </div>
    <div class="finding">
        <b>NDVI and NDBI dominate SHAP attribution</b> across all settlements, consistent with
        vegetation clearance and built-surface expansion being the primary physical change signal
        captured by CVA between the two epochs.
    </div>
    """, unsafe_allow_html=True)
